...
async function workingFix() {
  try {
    ...
    const checkResult = await prisma.$queryRaw`SELECT column_name FROM information_schema.columns WHERE table_name = 'AppConfig' AND column_name = 'gallery'`;
    ...
    // Update AppConfig
    await prisma.$executeRaw`UPDATE "AppConfig" SET "gallery" = ${jsonString}, "updatedAt" = NOW() WHERE "id" = 'default'`;

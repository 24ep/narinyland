...
/**
 * GET /api/instagram/image?url=https://www.instagram.com/p/XXXXX/
 * 
 * Fetches an Instagram post page, extracts the og:image meta tag,
 * and proxies the actual image back. This lets us display Instagram
 * photos as normal images without embeds or API tokens.
 */
router.get('/image', async (req: Request, res: Response) => {
  const igUrl = req.query.url as string;
  ...
  try {
    ...
    if (!ogImageMatch || !ogImageMatch[1]) {
      // Fallback: try to find any CDN image URL in the page
      const cdnMatch = html.match(/(https:\/\/[^"'\s]+(?:cdninstagram|fbcdn)[^"'\s]+\.jpg[^"'\s]*)/i);
      if (cdnMatch) {
        return proxyImage(cdnMatch[1], res);
      }
      return res.status(404).json({ error: 'Could not extract image from Instagram post' });
    }
    ...
    return res.status(500).json({ error: 'Failed to fetch Instagram image' });
  }
});

/**
 * Proxy an image URL — fetches the image and streams it to the client.
 * This avoids CORS issues and hotlink blocking.
 */
 ...
 * 
 * Scrapes a public Instagram profile page to extract recent post shortcodes.
 * Returns an array of Instagram post URLs that can be displayed via our image proxy.
 */
  ...
  try {
    ...
    try {
      ...
      if (apiResponse.ok) {
        ...
        if (user) {
          ...
          console.warn(`[Instagram Scraper] v1 API returned success but no user data for @${cleanUsername}`);
        }
      } else {
        const errorText = await apiResponse.text().catch(() => 'No error body');
        console.warn(`[Instagram Scraper] v1 API failed (${apiResponse.status}) for @${cleanUsername}. Error: ${errorText.substring(0, 100)}`);
      }
    } catch (apiErr: any) {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        return res.status(404).json({ error: `Profile @${cleanUsername} not found` });
      }
      throw new Error(`Instagram returned ${response.status}`);
    }
    ...
    // Debug: Check if we're hitting a login wall or a "sus" page
    if (html.includes('Login • Instagram') || html.includes('login/')) {
      console.warn(`[Instagram Scraper] Redirected to login page for @${cleanUsername}. Static scraping likely blocked.`);
    }
    ...
    // Filter out common false positives and short codes (real shortcodes are usually 11+ chars)
    ...
    res.status(500).json({ error: 'Failed to fetch Instagram profile. It may be private or Instagram blocked the request.' });
  }
});
